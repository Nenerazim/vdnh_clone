...
import type {IInfoEvent} from '@app/Interfaces/Page';

const show = ref(false);
defineProps<{component: IInfoEvent}>();
</script>
<template>
	<div class="main">
		<h1>{{ component.title }}</h1>
		<transition-group class="block-info" name="list" tag="div">
			<div v-for="(text, index) in component.settings.items" v-show="show || index < 1" :key="index" class="info">
				<div class="description">{{ text.title }}</div>
				<div>
					<div v-if="text.mark" class="mark">
						<div></div>
					</div>
